...
function localPayload(){
 ...
 const todo=(safeJson('romania-todo-v1',[])||[]).filter(x=>!deleted.has(String(x.id)));
...
async function deleteTodo(id){id=String(id||'');if(!id)return;const del=deletedTodoIds();del.add(id);localStorage.setItem(TODO_DELETED_KEY,JSON.stringify([...del]));const todo=(safeJson('romania-todo-v1',[])||[]).filter(x=>String(x.id)!==id);localStorage.setItem('romania-todo-v1',JSON.stringify(todo));if(!endpoint())return;setStatus('🔄 מוחק…','syncing');try{const res=await api({action:'delete',sheet:'Todo',ids:[id],keyField:'id'});if(!res.ok)throw new Error(res.error||'delete failed');localStorage.setItem(LAST_SYNC_KEY,new Date().toISOString());setStatus('🟢 נמחק ומסונכרן','ok')}catch(e){setStatus('🟡 נמחק במכשיר · סנכרון יושלם בהמשך','offline')}}
