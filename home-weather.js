...
const places=[
 ...
 {re:/otp|otopeni|klass|monopoly/i,name:'בוקרשט / אוטופן',lat:44.5711,lon:26.085}
];
...
function pickPlace(day){const stops=Array.isArray(day?.s)?day.s:[];const first=stops.find(s=>placeForStop(s));if(first&&/otp|otopeni|monopoly|bucharest/i.test(`${first?.[0]||''} ${first?.[4]||''}`))return placeForStop(first);const balea=stops.find(s=>/bâlea|balea/i.test(`${s?.[0]||''} ${s?.[4]||''}`));if(balea)return placeForStop(balea);const candidates=stops.filter(s=>!FIXED_RE.test(`${s?.[0]||''} ${s?.[4]||''}`));for(let i=candidates.length-1;i>=0;i--){const p=placeForStop(candidates[i]);if(p)return p}for(let i=stops.length-1;i>=0;i--){const p=placeForStop(stops[i]);if(p)return p}return null}
...
function ensureSection(){const page=document.getElementById('page-home'),dash=document.getElementById('realDashboard');if(!page||!dash)return null;let sec=document.getElementById('homeWeather');if(!sec){sec=document.createElement('section');sec.id='homeWeather';sec.className='dash-card';sec.innerHTML='<div class="dash-section-title"><h2>🌦️ מזג אוויר לפי המסלול</h2><small>מתעדכן לפי היעד המתוכנן בכל יום</small></div><div id="homeWeatherBody" class="weather-loading">טוען תחזית…</div>'}
 const todayTitle=[...dash.querySelectorAll('.dash-card h2')].find(h=>/מה בתוכנית היום/.test(h.textContent||''));
 const todayCard=todayTitle?.closest('.dash-card');
 if(todayCard?.parentElement?.classList.contains('dash-grid'))todayCard.parentElement.parentElement.insertBefore(sec,todayCard.parentElement);else dash.appendChild(sec);
 return sec}
