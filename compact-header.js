...
header .brand{font-size:.9rem!important;line-height:1.15!important;overflow:visible!important;text-overflow:ellipsis!important;white-space:nowrap!important;max-width:230px!important;display:inline-flex!important;align-items:center!important;gap:7px!important}
...
function removeLegacyStatus(){
 ...
 document.querySelectorAll('header div,header span,header p').forEach(el=>{
   if(el.id==='syncDot'||el.closest('#syncDot')||el.classList.contains('brand')||el.closest('.brand'))return;
   if(el.querySelector('button'))return;
   const t=(el.textContent||'').trim();
   if(/מסונכרן|מסנכרן|אופליין|הנתונים נשמרים במכשיר/.test(t))el.remove();
 });
}
...
function refresh(){shortenButtons();removeLegacyStatus();ensureDot()}
